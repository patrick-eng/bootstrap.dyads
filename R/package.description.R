...
#'
#' The bootstrap model removes a pre-defined proportion of random variables for each estimation (default = 20 percent), extracts the variable loading scores, and averages them across all trials.
#'
...
#'
#' Changes have been made to Stimson's original R code in order for the bootstrap model to inherrit and estimation the information it needs.
#'
#' Firstly, additional arguments have been added to the function itself including: data (define the object which holds the vectors), print (setting to FALSE allows the users to surpress all messages returned to the console), and log (default is true, generates a log file containing diagnostic information onto the user's hard-drive), and filename (character vector for where the log should be written to, default is "dyad-ratios-log.txt" under the working directory).
#'
...
#'
#' Fourthly, an argument named increase is added which allows uses to turn up the power of the exponential smoother applied to the aggregation process.
#'
#' Finally, a bug in which quarterly estimations would fail if a given record held certain dates (e.g. 30th May in any year) has been patched, as well as a bug which excluded the final observation in a database from being entered into the aggregation matrix.
#'
#' @keywords internal
#'
"_PACKAGE"
